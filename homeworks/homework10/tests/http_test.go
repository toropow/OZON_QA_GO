...
import (
	...
	"encoding/json"
	"fmt"
	routeclient "github.com/ozonmp/act-device-api/homeworks/homework10/route-client"
...
func Test_HttpServer_List(t *testing.T) {
	...
	t.Run("Remove devices via client API", func(t *testing.T) {
		...
		require.Equal(t, code.StatusCode, 200)
		require.Equal(t, responseRemoveDevice.Found, true)
	})
	...
	t.Run("Remove devices with empty Id via client API", func(t *testing.T) {
		...
		//Assert
		require.Nilf(t, err, "Error while executing the method: %s", err)
		require.Equal(t, code.StatusCode, 400)
		require.Equal(t, responseRemoveDevice.Found, false)
	})
	...
	t.Run("Update devices via client API", func(t *testing.T) {
		...
		require.Nilf(t, err, "Error while executing the method: %s", err)
		require.Equal(t, code.StatusCode, 200)
		require.Equal(t, responseUpdateDevice.Success, true)
	})
	...
	t.Run("Update isn't exists devices via client API", func(t *testing.T) {
		...
		require.Nilf(t, err, "Error while executing the method: %s", err)
		require.Equal(t, code.StatusCode, 200)
		require.Equal(t, responseUpdateDevice.Success, false)
	})
